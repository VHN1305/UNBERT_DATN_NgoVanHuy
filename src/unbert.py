...
class UNBertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        )  # tuple: (sequence_output, pooled_output, (hidden_states), (attentions))
        sequence_output = encoder_outputs[0]
        pooled_output = self.pooler(sequence_output)

        sentence_ids, sentence_mask, sentence_segment = sentence_input

        # print(sentence_ids.size())
        def reduce_mean(embed, sentence_id, weight=None):
            ...
            y = torch.zeros(
                (len(size), embed.size(-1)), dtype=torch.float, device=device
            )
            if weight is None:
                news_embed = y.index_add_(0, ind, embed) / (
                    size.float().unsqueeze(-1) + 1e-6
                )
            else:
                news_embed = y.index_add_(0, ind, embed)
        ...
        if news_mode == "mean":
            ...
            embed_flat = torch.reshape(
                sequence_output, (-1, sequence_output.size(1) * sequence_output.size(2))
            )
            embed_flat = embed_flat.clone()
            weights = att_mapping(embed_flat).unsqueeze(-1)
            sequence_output *= weights
            batch_data = [
                reduce_mean(embed, sentence_id, weight)
                for embed, sentence_id, weight in zip(
                    sequence_output, sentence_ids, weights
                )
            ]
            sentence_embedding = torch.cat(batch_data, 0)
        elif news_mode == "nseg":
            dummy = sentence_ids.unsqueeze(2).expand(
                sentence_ids.size(0), sentence_ids.size(1), sequence_output.size(2)
            )
        ...
        sentence_outputs = self.encoder_news(
            ...
            attention_mask=self.get_extended_attention_mask(
                sentence_mask, sentence_ids.size(), sentence_ids.device
            ),
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_extended_attention_mask,
